...
public class MapPostition {
    ...
    public MapPostition() {
        ...
        for (int i = 1; i < MAP_HEIGHT - 1; i++) mapOrder.add(
            new Position(MAP_WIDTH - 1, i)
        );
        for (int i = MAP_WIDTH - 1; i >= 0; i--) mapOrder.add(
            new Position(i, MAP_HEIGHT - 1)
        );
        for (int i = MAP_HEIGHT - 2; i > 0; i--) mapOrder.add(
            new Position(0, i)
        );
    }
}
